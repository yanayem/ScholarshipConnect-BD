...
export default function DiscussionDetailScreen() {
  ...
  const handleLike = async () => {
    if (!post || !currentUser) {
      Alert.alert('Login Required', 'Please login to like this discussion.');
      return;
    }
    ...
    const res = await apiService.likeDiscussion(post.id);
    if (!res.ok) {
      loadPost();
    }
  };
  ...
  const handleVote = async (optionId) => {
    ...
    const res = await apiService.voteDiscussion(post.id, optionId);
    if (res.ok) {
      setPost(res.data);
    }
  };
  ...
  const handleComment = async () => {
    ...
    let res;
    if (editingCommentId) {
      res = await apiService.updateDiscussionComment(editingCommentId, comment);
    } else {
      res = await apiService.commentDiscussion(post.id, comment);
    }

    if (res.ok) {
      setComment('');
      setEditingCommentId(null);
      showToast(editingCommentId ? 'Comment updated' : 'Comment added', 'success');
      loadPost();
    } else {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const res = await apiService.deleteDiscussionComment(commentId);
            if (res.ok) {
  ...
  const handleToggleSolved = async () => {
    try {
      const res = await apiService.updateDiscussion(post.id, { is_solved: !post.is_solved });
      if (res.ok) {
        const newState = !post.is_solved;
        setPost({ ...post, is_solved: newState });
        showToast(newState ? 'Marked as solved! +50 points' : 'Discussion reopened', 'success');
      }
    } catch (e) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const res = await apiService.deleteDiscussion(post.id);
            if (res.ok) {
  ...
  const totalVotes = post.poll_options?.reduce((acc, curr) => acc + curr.votes_count, 0) || 0;
    ...
    >
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.workspaceTag}>COMMUNITY WORKSPACE</Text>
          ...
          <View style={styles.infoSection}>
            ...
            <View style={styles.authorCard}>
              ...
              <View style={{ flex: 1, marginLeft: 16 }}>
                ...
                <Text style={styles.authorUni}>{post.category} • {new Date(post.created_at).toLocaleDateString()}</Text>
              </View>
            </View>

            <Text style={styles.contentTitle}>Topic Overview</Text>
            ...
            {post.poll_question && (
              <View style={styles.pollSection}>
                ...
                {post.poll_options?.map((opt) => {
                  ...
                  const isSelected = post.user_voted_option_id === opt.id;
                  return (
                    <TouchableOpacity
                      key={opt.id}
                    ...
                    >
                      ...
                      <View style={styles.pollOptionContent}>
                         <Text style={[styles.pollOptionText, isSelected && {fontWeight: 'bold'}]}>{opt.text}</Text>
                         <Text style={styles.pollVoteCount}>{percentage}%</Text>
                      </View>
                    </TouchableOpacity>
                  );
                })}
            ...
            <View style={styles.engagementBar}>
               <TouchableOpacity style={styles.statBtn} onPress={handleLike}>
                  <Ionicons name={post.is_liked ? "heart" : "heart-outline"} size={22} color={theme.colors.error} />
                  <Text style={[styles.statBtnText, post.is_liked && {color: theme.colors.error}]}>
                    {post.likes_count} {post.likes_count === 1 ? 'Person' : 'People'} Liked
                  </Text>
               </TouchableOpacity>
            </View>
          </View>
      ...
      {post.is_solved ? (
        <View style={[styles.interactionBar, { justifyContent: 'center', backgroundColor: theme.colors.background }]}>
          <MaterialIcons name="lock" size={20} color={theme.colors.textSecondary} />
          <Text style={{ marginLeft: 8, color: theme.colors.textSecondary, fontWeight: '600' }}>
...
const styles = StyleSheet.create({
  ...
  solvedBannerTitle: { fontSize: 16, fontWeight: 'bold', color: theme.colors.success },
  solvedBannerSub: { fontSize: 12, color: theme.colors.success, opacity: 0.8 },
  ...
  postImage: { width: '100%', height: 220, borderRadius: 12, marginTop: 15 },

  pollSection: { backgroundColor: theme.colors.surface, padding: 20, borderRadius: 20, ...theme.shadows.soft, marginTop: 5 },
  pollQuestion: { fontFamily: theme.typography.fontFamily.bold, fontSize: 16, color: theme.colors.heading, marginBottom: 15 },
  pollOptionBtn: { height: 44, borderRadius: 10, backgroundColor: theme.colors.background, marginBottom: 10, overflow: 'hidden', justifyContent: 'center', borderWidth: 1, borderColor: theme.colors.divider },
  pollOptionBtnSelected: { borderColor: theme.colors.primary, borderWidth: 2 },
  pollProgress: { position: 'absolute', left: 0, top: 0, bottom: 0, backgroundColor: theme.colors.primary + '20' },
  pollOptionContent: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 15, alignItems: 'center' },
  pollOptionText: { fontSize: 14, color: theme.colors.textPrimary },
  pollVoteCount: { fontSize: 12, color: theme.colors.textSecondary, fontWeight: 'bold' },
  pollTotalVotes: { fontSize: 11, color: theme.colors.textSecondary, textAlign: 'right', marginTop: 5 },
